...
description: >
  投研数字员工 — 基金诊断技能。
  对持有基金进行健康评估，分析持仓风险敞口、业绩归因与经理风格漂移。
  Use when: 帮我诊断一下我手里的易方达蓝筹精选, 这只基金最近风格漂移了吗？, 我的基金组合风险敞口大吗？
compatibility: "需要投研数字员工代理服务。使用前启动 backend/start_all.py。"
metadata:
  author: 投研数字员工
  version: "1.1.0"
  skill_type: warrenq_agent
  bot_id: "267723408540800"
  package: "@wyn/skills"
---
...
本技能通过「投研数字员工」代理服务调用 WarrenQ 智能体后端（bot_id: `267723408540800`）。
...
| 分析章节 | 内容说明 |
|---|---|
| 基金基本信息 | 类型、规模、费率、成立时间 |\n| 业绩归因 | 超额收益来源：选股/择时/行业 |\n| 持仓风险敞口 | 行业集中度、个股权重、估值水位 |\n| 基金经理风格 | 投资风格及一致性评估 |\n
...
curl.exe -N -s "${API}/api/tools/stream?skill=fund_diagnosis&q=$QUERY"
```
