...
description: >
  投研数字员工 — 国盛通AI 通用问答技能。
  金融领域通用问答兜底，当问题不属于八大专项智能体时使用。
  Use when: 今天天气怎么样？, 帮我写一段 Python 代码, 你是谁？能做什么？, 帮我解释一下市盈率是什么
compatibility: "需要投研数字员工代理服务。使用前启动 backend/start_all.py。"
metadata:
  author: 投研数字员工
  version: "1.1.0"
  skill_type: warrenq_agent
  bot_id: "247820468705408"
  package: "@wyn/skills"
---
...
本技能通过「投研数字员工」代理服务调用 WarrenQ 智能体后端（bot_id: `247820468705408`）。
...
| 分析章节 | 内容说明 |
|---|---|
| 通用金融常识 | 股票、基金、宏观等基础概念解答 |\n| 跨领域闲聊 | 与投资无关的日常问答 |\n| 兜底问答 | 无法匹配专项智能体时的安全入口 |\n
...
curl.exe -N -s "${API}/api/tools/stream?skill=gst_ai_chat&q=$QUERY"
```
