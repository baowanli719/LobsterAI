...
本技能通过「投研数字员工」代理服务调用 WarrenQ 智能体后端（bot_id: `273759415492736`）。
...
curl.exe -N -s "${API}/api/tools/stream?skill=movement_attribution&q=$QUERY"
```
...
```json
{"skill":"movement_attribution","q":"用户问题"}
```
