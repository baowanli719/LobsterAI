...
本技能通过「投研数字员工」代理服务调用 WarrenQ 智能体后端（bot_id: `248453604586624`）。
...
curl.exe -N -s "${API}/api/tools/stream?skill=smart_fund_picker&q=$QUERY"
```
...
```json
{"skill":"smart_fund_picker","q":"用户问题"}
```
