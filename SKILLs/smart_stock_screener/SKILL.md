...
本技能通过「投研数字员工」代理服务调用 WarrenQ 智能体后端（bot_id: `247829205446784`）。
...
curl.exe -N -s "${API}/api/tools/stream?skill=smart_stock_screener&q=$QUERY"
```
...
```json
{"skill":"smart_stock_screener","q":"用户问题"}
```
